...
fixup_brks() {
    ...
    for (i = 0; i < num; i++) {
        ...
        if (!is_wr_only) {
            ...
            tmp2 = tmp | BANK_IO_TMP | BANK_BREAK;SET_PAGE_INFO_RD(page, val - tmp + tmp2);
        }
        val = GET_PAGE_INFO_WR(page);
        tmp = PTR2WORD(val) & 0xff;
        tmp2 = tmp | BANK_IO_TMP | BANK_BREAK;SET_PAGE_INFO_WR(page, val - tmp + tmp2);
    }
}
...
fixup_any_bank_any_page(int start_page, int num_pages, byte *mem0rd,
    ...
    for (i = 0; i < num_pages; i++) { SET_PAGE_INFO_RD(i + start_page, mem0rd);
        mem0rd += 0x100;
    }

    for (i = 0; i < num_pages; i++) { SET_PAGE_INFO_WR(i + start_page, mem0wr);
        mem0wr += 0x100;
    }
...
fixup_intcx() {
    ...
    for (k = start_k; k < 4; k++) {
        ...
        off = off << 8;SET_PAGE_INFO_RD(0xc0 + off, SET_BANK_IO);
        ...
        for (j = 0xc0; j < 0xd0; j++) { SET_PAGE_INFO_WR(j + off, SET_BANK_IO);
        }
    }

    if (!no_io_shadow) { SET_PAGE_INFO_RD(0xc7, SET_BANK_IO);        /* smartport */
    }
...
fixup_wrdefram(int new_wrdefram) {
    ...
    for (j = 0x1e0; j < 0x200; j++) { SET_PAGE_INFO_WR(j, wrptr);
        wrptr += 0x100;
    }
    ...
    for (j = 0xe0; j < 0x100; j++) { SET_PAGE_INFO_WR(j, wrptr);
        wrptr += 0x100;
    }
    ...
    for (j = 0x1d0; j < 0x1e0; j++) { SET_PAGE_INFO_WR(j, wrptr);
        wrptr += 0x100;
    }
    ...
    for (j = 0xd0; j < 0xe0; j++) { SET_PAGE_INFO_WR(j, wrptr);
        wrptr += 0x100;
    }
...
fixup_altzp() {
    ...
    if (altzp) {
        mem0rd += 0x10000;
    }SET_PAGE_INFO_RD(0, mem0rd);SET_PAGE_INFO_RD(1, mem0rd + 0x100);SET_PAGE_INFO_WR(0, mem0rd);SET_PAGE_INFO_WR(1,
                                                                                                                  mem0rd +
                                                                                                                  0x100);
...
fixup_ramrd() {
    ...
    SET_PAGE_INFO_RD(2, mem0rd + 0x200);SET_PAGE_INFO_RD(3, mem0rd + 0x300);

    for (j = 8; j < 0x20; j++) { SET_PAGE_INFO_RD(j, mem0rd + j * 0x100);
    }

    for (j = 0x40; j < 0xc0; j++) { SET_PAGE_INFO_RD(j, mem0rd + j * 0x100);
    }
...
fixup_ramwrt() {
    ...
    SET_PAGE_INFO_WR(2, mem0wr + 0x200);SET_PAGE_INFO_WR(3, mem0wr + 0x300);
    ...
    for (j = 8; j < 0x0c; j++) { SET_PAGE_INFO_WR(j, mem0wr + j * 0x100 + shadow);
    }

    for (j = 0xc; j < 0x20; j++) { SET_PAGE_INFO_WR(j, mem0wr + j * 0x100);
    }
    ...
    for (j = 0x40; j < 0x60; j++) { SET_PAGE_INFO_WR(j, mem0wr + j * 0x100 + shadow);
    }
    ...
    for (j = 0x60; j < 0xa0; j++) { SET_PAGE_INFO_WR(j, mem0wr + j * 0x100 + shadow);
    }

    for (j = 0xa0; j < 0xc0; j++) { SET_PAGE_INFO_WR(j, mem0wr + j * 0x100);
    }
...
fixup_rdrom() {
    ...
    for (k = 0; k < 2; k++) {
        ...
        for (j = 0xe0; j < 0x100; j++) { SET_PAGE_INFO_RD(j + k * 0x100, mem0rd + j * 0x100);
        }
    }
...
fixup_shadow_txt1() {
    ...
    for (j = 4; j < 8; j++) { SET_PAGE_INFO_WR(0x100 + j, mem0wr + j * 0x100);
    }
}
...
fixup_shadow_txt2() {
    ...
    for (j = 8; j < 0xc; j++) { SET_PAGE_INFO_WR(j, mem0wr + j * 0x100);
    }
    ...
    for (j = 8; j < 0xc; j++) { SET_PAGE_INFO_WR(0x100 + j, mem0wr + j * 0x100);
    }
}
...
fixup_shadow_hires1() {
    ...
    for (j = 0x20; j < 0x40; j++) { SET_PAGE_INFO_WR(0x100 + j, mem0wr + j * 0x100);
    }
}
...
fixup_shadow_hires2() {
    ...
    for (j = 0x40; j < 0x60; j++) { SET_PAGE_INFO_WR(j, mem0wr + j * 0x100);
    }
    ...
    for (j = 0x40; j < 0x60; j++) { SET_PAGE_INFO_WR(0x100 + j, mem0wr + j * 0x100);
    }
}
...
fixup_shadow_shr() {
    ...
    for (j = 0x60; j < 0xa0; j++) { SET_PAGE_INFO_WR(j, mem0wr + j * 0x100);
    }
    ...
    for (j = 0x60; j < 0xa0; j++) { SET_PAGE_INFO_WR(0x100 + j, mem0wr + j * 0x100);
    }
}
...
update_shadow_reg(int val) {
    ...
    if ((xor_v & 0x20) && ((g_rom_version >= 3) || g_user_page2_shadow)) {
        fixup_shadow_txt2();
    }
    if (xor_v & 0x40) {
        fixup_shadow_iolc();
...
fixup_shadow_all_banks() {
    ...
    for (k = 3; k < num_banks; k += 2) {
        ...
        for (j = 0x20; j < 0xa0; j++) { SET_PAGE_INFO_WR(k * 0x100 + j, mem0rd);
            mem0rd += 0x100;
        }
    }
