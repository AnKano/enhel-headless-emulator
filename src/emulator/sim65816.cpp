...
memalloc_align(int size, int skip_amt, void **alloc_ptr) {
    ...
    bptr = static_cast<byte *>(calloc(size + skip_amt, 1));
    if (alloc_ptr) {
    ...
    if (prev_vbl_index == 0) {
        ...
        switch (g_limit_speed) {
            case 1:
                ...
                break;
            case 2:
                sp_str = strdup("2.8Mhz");
                break;
            case 3:
                sp_str = strdup("8.0Mhz");
                break;
            default:
                sp_str = strdup("Unlimited");
                break;
        }
        ...
        code_str2 = strdup("");
        if (g_code_yellow) {
            code_str1 = strdup("Code: Yellow");
            code_str2 = strdup("Emulated system state suspect, save work");
        }
        if (g_code_red) {
            code_str1 = strdup("Code: RED");
            code_str2 = strdup("Emulated system state probably corrupt");
        }
