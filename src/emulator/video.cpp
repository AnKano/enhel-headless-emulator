...
/* Initialize video system */
    ...
    for (i = 0; i < 200; i++) {
        g_a2_line_kimage[i] = static_cast<Kimage *>((void *) 0);
        g_a2_line_stat[i] = -1;
...
video_border_pixel_write(Kimage *kimage_ptr, int starty, int num_lines,
    ...
    for (i = 0; i < num_lines; i++) {
        ptr = (word32 *) &(kimage_ptr->data_ptr[(starty + i) * width_act * bytes_per_pix]);
...
video_get_kimages() {
    ...
    video_get_kimage(&g_kimage_superhires, 0, g_screen_depth, g_screen_mdepth);
    video_get_kimage(&g_kimage_border_special, 1, g_screen_depth, g_screen_mdepth);
    video_get_kimage(&g_kimage_border_sides, 2, g_screen_depth, g_screen_mdepth);
}
