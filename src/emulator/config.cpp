...
char *g_cfg_rom_path = strdup("ROM");
char *g_cfg_file_def_name = strdup("Undefined");
...
cfg_text_screen_dump() {
    ...
    filename = strdup("kegs.screen.dump");
...
config_load_roms() {
    ...
    for (i = 1; i < 8; i++) {
        ...
        if (g_cfg_tmp_path[0] != 0) {
            file = fopen(&(g_cfg_tmp_path[0]), "rb");
            if (file == 0) {
                fatal_printf("Open card ROM file %s failed: %d " "err:%d\n", &g_cfg_tmp_path[0], file, errno);
                continue;
            }
...
#else
            dsk->fd = open(uncomp_ptr, O_RDONLY, 0x1b6);
#endif
...
#else
        dsk->fd = open(name_ptr, O_RDWR , 0x1b6);
#endif
    }
...
#else
        dsk->fd = open(name_ptr, O_RDONLY, 0x1b6);
#endif
        can_write = 0;
    }
...
cfg_parse_menu(Cfg_menu *menuptr, int menu_pos, int highlight_pos, int change) {
    ...
    curstr = 0;
    if (type == CFGTYPE_INT) {
        iptr = static_cast<int *>(menuptr->ptr);
        curval = *iptr;
        iptr = static_cast<int *>(menuptr->defptr);
        defval = *iptr;
        if (curval == defval) {
...
cfg_file_add_dirent(Cfg_listhdr *listhdrptr, const char *nameptr, int is_dir,
    ...
    if (listhdrptr->last >= listhdrptr->max) {
        ...
        listhdrptr->direntptr = static_cast<Cfg_dirent *>(realloc(listhdrptr->direntptr,
                                                                  listhdrptr->max * sizeof(Cfg_dirent)));
    }
    ptr = static_cast<char *>(malloc(namelen + 1 + is_dir));
...
cfg_file_update_ptr(char *str) {
    ...
    len = strlen(str) + 1;
    newstr = static_cast<char *>(malloc(len));
